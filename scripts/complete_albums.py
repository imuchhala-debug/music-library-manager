...
def do_itunes_search(query):
    ...
    search_url = (
        f"https://itunes.apple.com/search?term={encoded_query}&entity=album&limit=15"
    )
...
def normalize_for_search(text):
    ...
    text = re.sub(
        r"\s*(Deluxe|Remaster|Edition|Version|Bonus|Extended|Explicit).*$",
        "",
        text,
        flags=re.IGNORECASE,
    )
    ...
    for query in search_queries:
        ...
        for result in data["results"]:
            ...
            if artist_lower == result_artist:
                ...
                word in result_artist for word in artist_lower.split() if len(word) > 2
            ):
                artist_score = 15
...
def download_apple_music_album(url):
    ...
    try:
        result = subprocess.run(
            ["gamdl", "--config-path", "config/gamdl.toml", "--overwrite", url],
            capture_output=True,
            text=True,
            cwd=MUSIC_DIR,
        )
...
def main():
    ...
    response = (
        input(f"\nDownload {len(albums_with_urls)} albums from Apple Music? [y/N]: ")
        .strip()
        .lower()
    )
    ...
    for album in albums_with_urls:
        ...
        if ok:
            ...
            if "Not in your Apple Music library" in str(error):
                print("   ⚠️ Not in library - skipped")
